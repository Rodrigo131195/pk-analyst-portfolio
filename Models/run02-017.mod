;; 1. Based on: run02-015
$PROBLEM 
;; 2. Description: MM + groups gow + Hill
;; x1. Author: munoz01r
...
halfT = THETA(16) * EXP(ETA(8))
n     = THETA(17) * EXP(ETA(9))
...
$DES
  ...
  DADT(5) = ADA_INPUT - K50 * A(5) - X * MM_BIND
...
$TABLE ID TIME TAD DV MDV EVID PRED IPRED IWRES CWRES CMTX VISIT WT ADA MTX ADA_G ONEHEADER NOPRINT FILE=sdtab02-017
$TABLE ID CL CLX KM VMAX X halfT n V2 V3 V4 V5 Q23 KA maxADA ETA1 ETA2 ETA3 ETA4 ETA5 ETA6 ETA7 ETA8 ETA9 ONEHEADER NOPRINT FILE=patab02-017
